...
import type { ITask, Priority, IReminder, IUser } from "./types.js";
...
const USERS_PATH: string = path.join(rootDir, "users.json");

const publicDir: string = path.join(__dirname, "..", "public");

const sessions = new Map<string, string>();

function readUsersFile(): IUser[] {
  if (!fs.existsSync(USERS_PATH)) {
    fs.writeFileSync(USERS_PATH, JSON.stringify([], null, 2), "utf-8");
  }
  try {
    const raw: string = fs.readFileSync(USERS_PATH, "utf-8");
...
function writeUsersFile(users: IUser[]): void {
...
function getUsernameFromReq(req: http.IncomingMessage): string | null {
...
function assignOwnerlessTasks(username: string): void {
  ...
  if (url.pathname.startsWith("/api/tasks")) {
    ...
    if (url.pathname === "/api/tasks") {
      if (method === "GET") {
        ...
        sendJson(res, 200, { tasks: userTasks.map((t: ITask) => serializeTask(t)) });
        return;
      }
    ...
    if (url.pathname.startsWith("/api/tasks/")) {
      ...
      if (method === "PATCH") {
        ...
        if (next.isCompleted && next.subtasks && next.subtasks.some((st: any) => !st.isCompleted)) {
      ...
      if (method === "DELETE") {
        ...
        const filtered = tasks.filter((t: ITask) => t.id !== id);
